...
from parser import get_teams, get_stat, get_matches
...
def pipeline():
    ...
    sorted_teams = sorted(teams.values(), key=lambda team: team.avg_total_corners, reverse=True)

    # 5. Сохраняем в csv формат
    logger.info("Сохраняем в csv формат")
    with open("corners.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["team", "avg_total_corners", "avg_team_corners", "avg_enemy_corners"])

        for team in sorted_teams:
            writer.writerow([team.ru_title, team.avg_total_corners, team.avg_team_corners, team.avg_enemy_corners])
